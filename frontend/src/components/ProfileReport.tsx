...
const ProfileReport = ({ report }: Props) => {
  ...
  return (
    <div
      ...
      key={report.id}
    >
      <p className="text-xl font-bold mb-5">{report.title}</p>
      <img
        src={getImage(report.image)}
        className="w-60 h-full mx-auto rounded-lg"
      />
      <p className="text-base my-5">{report.description}</p>
      <p className={`${color} font-bold text-lg`}>{label}</p>
      <div className="mb-4 flex flex-col gap-4 bg-slate-800 p-4 rounded-lg shadow-md mt-5 w-full">
